...
export default function App() {
  ...
  const content = useMemo(() => {
    switch (activeTab) {
      ...
      default:
        return (
          <DashboardScreen connected={connected} setConnected={setConnected} />
        );
    }
  }, [activeTab, connected]);
...
function DashboardScreen({
  ...
  const lowCount = [
    ...inventory.filter((item) => item.count / item.max < 0.1),
    ...fluids.filter((fluid) => fluid.volume / fluid.maxVolume < 0.1),
  ].length;
        ...
        <View style={styles.slotGrid}>
          {inventory.map((slot) => {
            ...
            return (
              <View
                key={slot.id}
                style={[styles.slotCard, isLow && styles.slotCardLow]}
              >
                <View
                  style={[styles.pillCircle, isLow && styles.pillCircleLow]}
                >
        ...
        <View style={styles.fluidStack}>
          {fluids.map((fluid) => {
            ...
            return (
              <View
                key={fluid.id}
                style={[styles.fluidCard, isLow && styles.fluidCardLow]}
              >
                <View style={styles.rowBetween}>
                  ...
                  <Text
                    style={[styles.fluidValue, isLow && styles.slotCountLow]}
                  >
                    {fluid.volume}
                    {fluid.unit}
                  </Text>
                ...
                <View style={styles.progressTrack}>
                  <View
                    style={[
                      styles.progressFill,
                      { width: `${(fluid.volume / fluid.maxVolume) * 100}%` },
                    ]}
                  />
                </View>
              </View>
                  ...
                  <View style={styles.rowBetween}>
                    ...
                    <View
                      style={[
                        ...
                        done && styles.timelineBadgeDone,
                        active && styles.timelineBadgeNext,
                      ]}
                    >
                      <Text style={styles.timelineBadgeText}>
                        {done ? "Dispensed" : active ? "Next Up" : "Scheduled"}
                      </Text>
                    </View>
                  </View>
    ...
    <View style={styles.stack}>
      <View style={styles.card}>
        ...
        <View style={styles.logStack}>
          {logEntries.map((event) => (
            <View
              key={event.ts}
              style={[styles.logCard, !event.ok && styles.logCardAlert]}
            >
              <View style={styles.rowBetween}>
                ...
                <View
                  style={[
                    styles.logBadge,
                    event.ok ? styles.logBadgeOk : styles.logBadgeAlert,
                  ]}
                >
                  <Text
                    style={[
                      styles.logBadgeText,
                      !event.ok && styles.logBadgeTextAlert,
                    ]}
                  >
              ...
              <Text style={styles.sectionMeta}>
                Beam: {event.beam} pills · {event.actual} / {event.expected}
              </Text>
            </View>
          ))}
        </View>
      ...
      <View style={[styles.heroCard, styles.heroCardDark]}>
        ...
        <View style={styles.timelineStack}>
          <TimelineStep
            step="1"
            label="Dose dispensed — shield opens"
            delay="T+0"
            accent={colors.teal}
          />
          <TimelineStep
            step="2"
            label="No collection detected → SMS blast"
            delay="T+15min"
            accent={colors.orange}
          />
          <TimelineStep
            step="3"
            label="Second SMS blast + emergency advisory"
            delay="T+30min"
            accent={colors.orange}
          />
        </View>
...
function TimelineStep({
  ...
  return (
    <View style={styles.timelineStep}>
      <View
        style={[styles.timelineStepBadge, { backgroundColor: `${accent}22` }]}
      >
        <Text style={[styles.timelineStepNumber, { color: accent }]}>
          {step}
        </Text>
      </View>
      <View style={{ flex: 1 }}>
        <Text style={[styles.timelineStepDelay, { color: accent }]}>
          {delay}
        </Text>
